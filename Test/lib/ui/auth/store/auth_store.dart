...
abstract class _AuthStoreBase with Store {
  ...
  @observable
  Color? buttonColor = AppColor.lightGrey;

  @observable
  BaseResponse<OtpResModel>? otpRes;
  ...
  @observable
  BaseResponse<SocialProfileResModel>? socialProfileRes;
  ...
  // @action
  // Future sendOtp(String phoneNo) async {}
  @action
  ...
  @action
  Future otpSend(OtpReqModel data) async {
    try {
      isloading = true;
      errorMessage = null;
      otpRes = await authRepo.sendOtp(data);
    } on DioException catch (dioError) {
      errorMessage = DioExceptionUtil.handleError(dioError);
      throw DioExceptionUtil.handleError(dioError);
    } on AppException catch (e) {
      errorMessage = e.toString();
      throw Exception(e);
    } finally {
  ...
  @action
  Future getProfile(SocialProfileReqModel data) async {
    try {
      isloading = true;
      errorMessage = null;
      socialProfileRes = await authRepo.getProfile(data);
    } on DioException catch (dioError) {
      errorMessage = DioExceptionUtil.handleError(dioError);
      throw DioExceptionUtil.handleError(dioError);
    } on AppException catch (e) {
